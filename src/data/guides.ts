...
export const GUIDES: Guide[] = [
  {
    ...
    sections: [
      ...
      { h2: 'Why the coast moves everything up a notch', html: `<p>Turf-care advice written for Phoenix or Denver assumes the infill dries out between rains. On the Grand Strand it does not, from spring through fall. Damp infill keeps odor bacteria active and grows algae in the shade, and the spring pollen load adds a layer of organic matter every year that the dry-climate advice never mentions. So a yard that would be fine with one clean a year in Arizona wants two here, and a one-dog yard that could go twice a year inland wants quarterly on the coast.</p>` },
  ...
  },
  {
    ...
    sections: [
      ...
      { h2: 'One-time clean or membership?', html: `<p>Run the numbers on your yard. A one-dog, 800-square-foot yard cleaned twice a year with Premium Restorations is ${money(P.rows[1].premium! * 2)}. The same yard on Essential Care is ${money(P.memberships[0].monthly * 12)} a year for four visits with priority scheduling. For a yard that needs the odor treatment every time, TIMELESS ELITE at ${money(P.memberships[1].monthly * 12)} a year buys four Premium Restorations that would cost ${money(P.rows[1].premium! * 4)} bought one at a time. The membership is cheaper if you would otherwise book three or more visits a year; a one-time clean is cheaper if you would only book one. Our <a href="/guides/how-often-to-clean-artificial-turf/">frequency guide</a> tells you which you are.</p>` },
